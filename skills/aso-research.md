...
- **File exists and `--refresh` is NOT set:** Read the first line of the file to extract the recorded locale (format: `> Locale: <locale>`).
  - If the first line does not match the `> Locale:` format (e.g. file was generated by an older version): inform the user:
    > `ASO_RESEARCH.md` has no locale record. Re-running analysis for `<current-locale>`.
